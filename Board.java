...
/** Represents the state of a JumpCube game.  Squares are indexed either by
...
abstract class Board {
    ...
    boolean isLegal(Color player, int r, int c) {
        return player.playableSquare(color(r, c));
    }
    ...
    final Color getWinner() {
        int n = size();
        if (numOfColor(BLUE) == n * n) {
            return BLUE;
        } else if (numOfColor(RED) == n * n) {
            return RED;
        } else {
            return null;
        }
        ...
        for (int i = 1; i <= size(); i++) {
            ...
            for (int j = 1; j <= size(); j++) {
                if (spots(i, j) < 1) {
                    ...
                    temp += " " + spots(i, j)
                        + color(i, j).toString().substring(0, 1);
                }
            }
            out += temp + "\n";
        }
